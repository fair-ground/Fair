...
final class FairAppTests: XCTestCase {
    ...
    func testCatalogLocalizations() async throws {
        // TODO: update to use the new localization format
        let item = AppCatalogItem(name: "ABC", localizations: ["fr-FR": AppCatalogItem(name: "XYZ")])
        XCTAssertEqual("XYZ", item.localize(key: \.name, into: "fr-FR"))
        XCTAssertEqual("ABC", item.localize(key: \.name, into: "????"))
